...
class SpatialTemporalFieldImageStateNineActions(gym.Env):
    """
    ...
    def __init__(
        ...
        super(SpatialTemporalFieldImageStateNineActions,
              self).__init__()
    ...
    def step(self, action_id):
        ...
        next_state = OrderedDict(
            [
                ("gradients", concentration_states),
                ("field", self.agent_field.get_viewscope_image()),
            ]
        )
    ...
    def calculate_step_reward(self, next_pos, hit_wall):
        return self.agent_field.vs_field_improvement / 25

    def calculate_done_reward(self):
        return np.sum(self.agent_field.vs_field_raw)/np.sum(self.env_field.field)*100
    ...
    def view_env_state(self, episode_num, timestep, save=False, path=None):
        ...
        fig_learning_axes[0, 1].imshow(
            self.agent_field.get_viewscope_image().squeeze(), cmap=cmap_color)
